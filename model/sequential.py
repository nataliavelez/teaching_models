...
class Problem: 
    ...
    def sequential(self, nIter): 
        """return pragmatic h|d and d|h after nIter iterations"""
        hGd_seq = []
        dGh_seq = []

        # initialize row sums and col sums
        r = [1, 1, 1, 1]
        c = [1, 1, 1, 1]
        ...
        for ex in self.exs:
            ...
            for _ in range(nIter):
                df_d = df_h.div(df_h.sum(axis=0).replace(0, np.nan), axis=1) * c # P(d|h)
                df_h = df_d.div(df_d.sum(axis=1).replace(0, np.nan), axis=0) * r # P(h|d)



            hGd_seq.append(df_h.fillna(0))
            dGh_seq.append(df_d.fillna(0))
            
            c = 36 * df_d.xs(ex).to_numpy()


        self.hGd_seq = hGd_seq
        self.dGh_seq = dGh_seq

        return self.hGd_seq, self.dGh_seq

    def runModel(self, nIter): # needs self.exs to be populated
        self.example_space()
        self.literal()
        self.pragmatic(nIter)
        self.sequential(nIter)
        return None
